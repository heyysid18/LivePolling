...
function setupSocket(io) {
    ...
    PollService_1.PollService.on('timer_sync', (remainingTime) => {
        io.emit('timer_sync', remainingTime);
    });
    PollService_1.PollService.on('poll_results', (completedPoll) => {
        io.emit('poll_results', completedPoll);
    });
    io.on('connection', (socket) => {
        console.log('Client connected:', socket.id);
        // Student/Teacher joins or reconnects
        socket.on('join_poll', async (data) => {
            try {
                const activePoll = await PollService_1.PollService.getActivePoll();
                if (activePoll) {
                    socket.emit('poll_started', activePoll);
                    // Resiliently calculate remaining time from the server start time
                    socket.emit('timer_sync', PollService_1.PollService.getRemainingTime(activePoll));
                    // If student refreshes and had already voted, resume their state
                    if (data.studentId && data.role === 'student') {
                        const vote = await PollService_1.PollService.getStudentVote(activePoll._id.toString(), data.studentId);
                        if (vote) {
        ...
        socket.on('create_poll', async (data) => {
            ...
            try {
                const poll = await PollService_1.PollService.createPoll(data.question, data.options, data.duration, data.createdBy);
        ...
        socket.on('end_poll', async (data) => {
            try {
                const poll = await PollService_1.PollService.endPoll(data.pollId);
                if (poll) {
        ...
        socket.on('submit_vote', async (data) => {
            try {
                const updatedPoll = await PollService_1.PollService.castVote(data.pollId, data.studentId, data.studentName, data.selectedOption);
